...
for infile in listing:
	file_list.append(infile.replace('.Classify.otu.level4.taxonomy.txt', ''))
...
cursor.execute('''CREATE TABLE Reads(Taxon Unique, Class1)''')
...
for infile in glob.glob(os.path.join(path, '*.txt')):
	with open(infile) as OTUs:
		patient = str(infile).replace('.Classify.otu.level4.taxonomy.txt', '').replace(path, '').replace('\\', '')
		for line in OTUs:
			...
			if OTU_size == 'Size':
				...
				taxa = OTU_tax.split(';')
				OTU_taxon = taxa[2]
				OTU_tax = taxa[0] + "_" + taxa[1] + "_" + taxa[2]
				OTU_tax = OTU_tax.replace('\"','')
				sql1 = "INSERT OR IGNORE INTO Reads (Taxon, " + patient + ", Class1) VALUES (?, ?, ?)"
...
csv_writer = csv.writer(open("c:\Sync Folder\Biorg\Python\Class_Table.csv", "wt"))
